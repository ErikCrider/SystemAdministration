## This is a modification of Get-RecycleBin.ps1 created by Brian Wilhite with the following modifications
## Updated for Windows 10
## Set to only delete files older than 60 days (configurable on line 134)

Function Get-RecycleBinSize60Days
{
<#
.SYNOPSIS
...
EDITs - Erik Crider
Updated script 10/13/2020
Cast this as an integer because Win10 versions return a string
OLD - Switch ($WMI_OS.BuildNumber)
NEW - Switch ([int]$WMI_OS.BuildNumber) 

Changed to only delete last 60 days as this suits our business needs.
OLD - Remove-Item -Path $Delete -Exclude "desktop.ini" -Force -Recurse
NEW - Get-ChildItem  -Path $Delete -recurse -force | Where-Object {($_.LastWriteTime -lt (Get-Date).AddDays(-60))} | Remove-Item -Exclude "desktop.ini" -Force -Recurse
          ...
          Foreach ($Bin in $Bins)
           {
            If ($Empty)
             {
              $Delete = $Bin.FullName + "\*"
              Get-ChildItem  -Path $Delete -recurse -force | Where-Object {($_.LastWriteTime -lt (Get-Date).AddDays(-60))} | Remove-Item -Exclude "desktop.ini" -Force -Recurse
             }#End If ($Empty)
